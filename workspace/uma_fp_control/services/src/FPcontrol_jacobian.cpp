...
/**
 * @brief Hybrid position + tissue force + abdomen force control pipeline.
 *
 * Implements the parallel scheme from the block diagram. Three loops run
 * every cycle and their velocity outputs are summed before speedl():
 *
 *   Loop 1 — Tissue force limiter (admittance, modifies setpoint):
 *     dZ = K_tissue * max(0, Fz - threshold)    → corrects desPose.z upward
 *     The position controller sees a raised Z target when tissue force is high.
 *
 *   Loop 2 — Position control (RCM + PID):
 *     T_dest = RCMGeometry(fulcrum, desPose + dZ)
 *     v_pos  = PID(error(T_dest, T_TTP))        → 6-DOF velocity
 *
 *   Loop 3 — Abdomen force controller (P/PI, adds velocity directly):
 *     v_abd  = K_abdomen * (0 - f_abdomen)       → 3-DOF flee velocity
 *
 *   Final: v_cmd = v_pos + v_abd  (linear components summed; angular from pos only)
 *
 * The fulcrum is still estimated every cycle (updateFulcrumState) so the RCM
 * geometry always uses the best available estimate regardless of contact state.
 *
...
std::vector<double> hybridControl::runHybridControl(){
    ...
    // --- Loop 3: Abdomen force controller → flee velocity ---
    // Runs every cycle. When forces are below threshold, returns {0,0,0}.
    //std::cout << "forceAbdomen=[" << base_forceAbdomen[0] << ", " << base_forceAbdomen[1] << ", " << base_forceAbdomen[2] << "]" << std::endl;
    std::vector<double> v_abd = abdomenForceCtrl_.compute(base_forceAbdomen);
    //v_abd = {0,0,0};
    // --- Sum linear velocities; angular comes from position control only ---
    // ❗ Negated wx and zeroed wz — verify sign convention for this robot
    return { velVector[0] + v_abd[0],
             velVector[1] + v_abd[1],
             velVector[2] + v_abd[2],
            -velAngular(0),
             velAngular(1),
...
/**
 * @brief Updates fulcrum_position every cycle from two non-exclusive sources:
 *
 *   1. Coordinator callback (newFulcrum flag): external estimate (vision/geometry).
 *   2. Kinematic recomputation: always runs, keeping the estimate consistent
 *      with the current robot pose. When abdominal forces are present the robot
 *      may have moved, so this ensures the RCM geometry uses a fresh value.
 *
 * Running both sources every cycle (not as exclusive branches) means the
 * abdomen force flee-velocity and the fulcrum correction happen in the same
 * cycle, fed into the same position controller.
 */
void hybridControl::updateFulcrumState(){
    if (newFulcrum) {
        std::cout << "....................................." << std::endl;
        fulcrum_position = computeFulcrum(E_T_Fp, T_E);
        std::cout << "jacob fulcrum = ["<< fulcrum_position.x << ", " << fulcrum_position.y << ", " << fulcrum_position.z << "]" << std::endl;
        newFulcrum = false;
    }
    if (!isAbdomenSafe()) {
...
std::vector<double> hybridControl::forceControl(const double& Kf, std::vector<double> forces){
    return { Kf*forces[0], Kf*forces[1], Kf*forces[2] };
}
...
int main(int argc, char **argv){
    ...
    std::cout << "Entering JACOBIAN control loop at 125 Hz" << std::endl;
